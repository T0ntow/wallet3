...
export class CategoriesService {
  ...
  async updateCategory(id: number, name: string, icon: string, tipo: string): Promise<void> {
    const db = await this.databaseService.getDb();
    ...
    const sql = `UPDATE categories SET name = ?, icon = ?, tipo = ? WHERE id = ?`;

    try {
      await db.run(sql, [name, icon, tipo, id]);
